...
proc newSyncFunctionsClient*(url, apiKey: string): SyncFunctionsClient {.inline.} =
  SyncFunctionsClient(url: url, client: newHttpClient(userAgent="supabase-nim/functions-nim v" & NimVersion,
    headers=newHttpHeaders({"Content-Type": "application/json", "Authorization": "Bearer " & apiKey})
  ))

proc newAsyncFunctionsClient*(url, apiKey: string): AsyncFunctionsClient {.inline.} =
  AsyncFunctionsClient(url: url, client: newAsyncHttpClient(userAgent="supabase-nim/functions-nim v" & NimVersion,
    headers=newHttpHeaders({"Content-Type": "application/json", "Authorization": "Bearer " & apiKey})
  ))

proc invoke*(self: SyncFunctionsClient | AsyncFunctionsClient; functionName: string, body = ""; responseType = SFResponseType.Json; region = SFRegion.Any; httpMethod = HttpPost; apiKey = ""): auto =
  ...
  result = self.client.request(url = self.url & '/' & functionName, httpMethod = httpMethod, body = body)
